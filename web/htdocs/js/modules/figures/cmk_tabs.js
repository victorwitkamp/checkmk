...
export class TabsBar {
    ...
    initialize() {
        this._defines = this._div_selection.append("svg")
            .attr("height", 0)
            .attr("width", 0)
            .style("position", "absolute")
            .style("margin-left", "-100%");

        this._defines.append("path")
            .attr("id", "tab-shape")
            .classed("tab-shape", true)
            .attr("d", `M116.486,29.036c-23.582-8-14.821-29-42.018-29h-62.4C5.441,0.036,0,5.376,0,12.003v28.033h122v-11H116.486
			z`);


        this._nav = this._div_selection.append("nav")
            .attr("role", "navigation")
            .classed("main-navigation", true);

        this.main_content = this._div_selection.append("div")
            .classed("main-content", true);

        this._register_tabs();

        this._ul  = this._nav.append("ul");
        let a_selection = this._ul.selectAll("li").data(this._tabs_list).enter()
            .append("li")
            .each((d, idx, nodes)=>{
                d3.select(nodes[idx])
                    .classed(d.tab_id(), true);
            })
        ...
        a_selection.append("span")
            .classed("noselect", true)
            .text(d=>d.name());
        a_selection.append("svg")
            .attr("viewBox", "0 0 122 40")
            .append("use")
...
export class Tab {
    constructor(tabs_bar) {
        ...
        this._tab_selection = tabs_bar.main_content.append("div")
            .attr("id", this.tab_id())
