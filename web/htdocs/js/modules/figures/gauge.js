...
// WIP, Gauge function will be replaced by a better implementation
class GaugeFigure extends cmk_figures.FigureBase {
    ...
    initialize() {
        ...
        this.scheduler.set_update_interval(1);
        this.set_post_url_and_body("random_data.py");
    ...
    _create_gauges() {
        this._create_gauge("memory", "Memory");
        this._create_gauge("cpu", "CPU");
        this._create_gauge("network", "Network");
    }

    _create_gauge(name, label, min, max) {
        var config = {
            size: 120,
            label: label,
        ...
        let new_gauge = new Gauge(this._div_id, config);
        new_gauge.render();
        this._scope.gauges.push(new_gauge);
    }

    update_gui() {
        this._scope.gauges.forEach(gauge=>{
            gauge.redraw(this.get_random_value(gauge));
        });
    }

    get_random_value(gauge) {
        var overflow = 0; //10;
...
// https://gist.github.com/zcross/4063645
function Gauge(placeholderName, configuration)
{
    ...
    this.render = function()
    {
        ...
        for (var major = this.config.min; major <= this.config.max; major += majorDelta)
        {
            ...
            {
                var point1 = this.valueToPoint(minor-this.config.min, 0.75);
                var point2 = this.valueToPoint(minor-this.config.min, 0.85);
            ...
            var point1 = this.valueToPoint(major-this.config.min, 0.7);
            var point2 = this.valueToPoint(major-this.config.min, 0.85);
            ...
            {
                var point = this.valueToPoint(major-this.config.min, 0.63);
